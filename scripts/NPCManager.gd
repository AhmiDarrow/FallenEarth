...
## NPCs are allies/neutral only. Enemies (mobs) are handled by EncounterBuilder + MobManager.
extends Node
...
func generate_for_world(world_seed: String, tile_map: Dictionary, start_tile_key: String) -> Dictionary:
	...
	print("[NPCManager] Generated %d NPC(s) for seed '%s'." % [_world_npcs.size(), world_seed])

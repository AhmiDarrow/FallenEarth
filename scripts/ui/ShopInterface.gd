...
const DEFAULT_STOCK := [
	...
	{"item_id": "stick", "count": 20, "buy_price": 2},
	{"item_id": "stone", "count": 20, "buy_price": 2},
	{"item_id": "iron_ore", "count": 10, "buy_price": 8},
...
func _build_ui() -> void:
	...
	var stock_list := UH.make_vbox(0, true, false)
	stock_list.name = "StockList"
	stock_scroll.add_child(stock_list)

	# Inventory column
	var inv_vbox := UH.make_vbox(0, true, false)
	inv_vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	body.add_child(inv_vbox)
	var inv_label := UH.make_label("Your inventory (sell):", 14, MT.TEXT_DANGER)
	inv_vbox.add_child(inv_label)
	var inv_scroll := UH.make_scroll_container()
	...
	var inv_list := UH.make_vbox(0, true, false)
...
func _populate_stock() -> void:
	...
	for stock in DEFAULT_STOCK:
		...
		var info := UH.make_label("%s x%d  (%d EC each)" % [
			str(stock.get("item_id", "?")), int(stock.get("count", 1)),
			int(stock.get("buy_price", 1))
		])
		info.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		row.add_child(info)
		var buy_btn := UH.make_button("Buy")
		buy_btn.pressed.connect(_on_buy_pressed.bind(stock.get("item_id", ""), int(stock.get("buy_price", 1))))
...
func _populate_inventory() -> void:
	...
	if inv == null:
		return
	var snapshot: Array = inv.get_grid_snapshot()
	if snapshot.is_empty():
		var ph := UH.make_muted_label("(inventory empty)")
	...
	for slot in snapshot:
		...
		var info := UH.make_label("%s x%d" % [str(slot.get("item_id", "?")), int(slot.get("qty", 0))])
		info.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		row.add_child(info)
		var sell_price: int = _sell_price_for(str(slot.get("item_id", "")))
		var sell_btn := UH.make_button("Sell (%d EC)" % sell_price)

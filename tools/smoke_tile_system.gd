...
func _test_local_map_view() -> void:
	...
	# TERRAIN_GROUND as documented in v0.4.0 Phase 0. In a size=4 map,
	# the cell at terrain[4] is at (x=0, y=1) — the formula is
	# y * size + x = 4 → y=1, x=0.
	...
	# Verify the legacy rift_scar cell (0,1) renders the same as a ground cell
	# — its atlas coord should be Vector2i(0, TERRAIN_GROUND), not the raw rift_scar value.
	var ground2: TileMapLayer = view.get_ground_layer()
	var legacy_atlas: Vector2i = ground2.get_cell_atlas_coords(Vector2i(0, 1))
	if legacy_atlas != Vector2i(0, TileSetSvc.TERRAIN_GROUND):
		_fail("LocalMapView: legacy rift_scar (terrain=4) at (0,1) did not normalize to ground; got %s" % legacy_atlas)
		return
